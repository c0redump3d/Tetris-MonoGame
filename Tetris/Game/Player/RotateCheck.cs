...
namespace Tetris.Game.Player
{
    public class RotateCheck
    {
            ...
            if (IsOutOfBounds())
            {
                ...
                for (var g = 0; g < 5; g++)
                {
                    for (var i = 0; i < 4; i++)
                    {
                        ...
                        if (!IsOutOfBounds())
                        {
                            DebugConsole.Instance.AddMessage($"passed:{FromTo(checkPosAt, clockwise, iBlock)},{i}");
                        ...
                        UpdateXY();
                        UpdateRectangles();
                        DebugConsole.Instance.AddMessage($"failed:{i}");
                    }
        ...
        public bool TSpinLock()
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                switch (i)
                {
                    case 0:
                        ...
                        if (!IsOutOfBounds())
                        {
                            DebugConsole.Instance.AddMessage($"T-Spin failed at check {i} (X+32)");
                            return false;
                        }
                    ...
                    case 2:
                        ...
                        if (!IsOutOfBounds())
                        {
                            DebugConsole.Instance.AddMessage($"T-Spin failed at check {i} (Y+32)");
                            return false;
                        }
            ...
            DebugConsole.Instance.AddMessage("T-Spin lock detected!");
            return true;
        }
        ...
        private bool IsOutOfBounds()
        {
            ...
            for (var i = 0; i < boxes.Length; i++)
            {
                ...
                {
                    DebugConsole.Instance.AddMessage($"Failed at x check, {i},{boxes[i].X}");
                    return true;
                }

                if (boxes[i].Y > Globals.MaxY) // if we are off screen vertically
                {
                    DebugConsole.Instance.AddMessage("Failed at y check");
                    return true;
                }
                ...
                {
                    DebugConsole.Instance.AddMessage("Failed at box check");
                    return true;
                }
            }
        ...
        public static RotateCheck Instance
        {
            get
            {
                ...
                if (result == null)
                {
                    result = _instance ??= new RotateCheck();
                }
