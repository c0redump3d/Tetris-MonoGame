    ...
    public class KeyboardInput
    {
        ...
        public void HandleKeyPress(GameTime gameTime)
        {
            ...
            if (Gui.Instance.CurrentScreen is GuiInGame &&
                ...
                {
                    if (NetworkManager.Instance.IsServer() && NetworkManager.Instance.Connected || !NetworkManager.Instance.Connected)
                    {
            ...
            {
                DebugMenu.Instance.ShowMenu();
            }
            if (IsKeyPress(Keys.F5))
            {
                Gui.Instance.SetCurrentScreen(new GuiEditColor());
            }
            if (IsKeyPress(Keys.F6))
            {
                MessageAnimator.Instance.AddMessage(new Message(Globals.Hoog48, "Tetris!dkjfhaksdljhfksdhafhlsdkjhfkasdhfjkhldkahfkjdshkfhasdkfhlkdashfklasdhkjfl", Color.Blue));
            }
            ...
            if (IsKeyPress((Keys) GameSettings.Instance.GetOptionValue("HardDrop")) && !InGameManager.Instance.Waiting)
                PlayerController.Instance.HardDrop = true;

            if (IsKeyPress((Keys)GameSettings.Instance.GetOptionValue("Hold"))) HoldShape.Instance.SetHoldShape(Rotate.Instance.GetCurShape());

            if (keyState.IsKeyDown((Keys)GameSettings.Instance.GetOptionValue("Left")) && !oldKeyState.IsKeyDown((Keys)GameSettings.Instance.GetOptionValue("Right")))
            {
                if (elapsedTime[0] <= 0)
                {
            ...
            if (keyState.IsKeyDown((Keys)GameSettings.Instance.GetOptionValue("Right")) && !oldKeyState.IsKeyDown((Keys)GameSettings.Instance.GetOptionValue("Left")))
            {
                if (elapsedTime[1] <= 0)
                {
            ...
            if (IsKeyPress((Keys)GameSettings.Instance.GetOptionValue("RotateRight")) || IsKeyPress((Keys)GameSettings.Instance.GetOptionValue("RotateLeft")))
            {
                Rotate.Instance.RotateTetris(!keyState.IsKeyDown((Keys)GameSettings.Instance.GetOptionValue("RotateLeft")));
            }
        ...
        public static KeyboardInput Instance
        {
            get
            {
                ...
                if (result == null)
                {
                    result = _instance ??= new KeyboardInput();
                }
