...
namespace Tetris.Main.Player
{
    public class NextShape
    {
        ...
        private int whereAt = 0;

        /// <summary>
        /// This will set the shape of the next tetris block picture box.
        /// </summary>
        /// <param name="_spriteBatch"></param>
        public void RenderNextShapes(SpriteBatch _spriteBatch)
        {
            //624,53
            _spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.LinearClamp, DepthStencilState.Default, RasterizerState.CullCounterClockwise, null, Matrix.CreateTranslation(new Vector3(624, 53, 0)));
            for (int i = 0; i < 4; i++)
            {
                _spriteBatch.Draw(nextTetImage[0], nOne[i], Color.White);
                _spriteBatch.Draw(nextTetImage[1], nTwo[i], Color.White);
                _spriteBatch.Draw(nextTetImage[2], nThree[i], Color.White);
                _spriteBatch.Draw(nextTetImage[3], nFour[i], Color.White);
                _spriteBatch.Draw(nextTetImage[4], nFive[i], Color.White);
            }
            _spriteBatch.End();
        }
        ...
        {
            int countX = count != 0 ? 7 : 0;
            int countY = count != 0 ? getNextY(count) : 0;

            if (shape == 1)
            {
                nX = 34 + countX; nY = 50 + countY;
            ...
            {
                nX = 17 + countX + (count != 0 ? 8 : 0); nY = 20 + countY + (count != 0 ? 8 : 0);
                rects[0] = new Rectangle(nX, nY, size, size); // player controlled rect
                rects[1] = new Rectangle(nX + size, nY + size, size, size);
