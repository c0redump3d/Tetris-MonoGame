...
namespace Tetris.GUI.Particle.Particles
{
    public class Circle : Particle
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (wait < 0f)
            {
                X += (float) (Math.Cos(Size * (this.X)));
                Y += (float) (Math.Cos(Size * (this.Y)));
                wait = 1;
            }
        ...
        {
            spriteBatch.Draw(Globals.BallTexture, new Vector2(X, Y), null, Color.White * (Size < 0.5F ? Size : 0.5F), 0.0f, Vector2.Zero,
