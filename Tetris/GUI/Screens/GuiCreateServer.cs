...
public class GuiCreateServer : GuiScreen
{
    ...
    {
        base.SetUp();
        var running = NetworkManager.Instance.IsServer();
        AddControl(new Button(new Vector2(640, 510), running ? "Stop Server" : "Start Server", Fonts.Hoog48));
    ...
    private void TryConnect(object sender)
    {
        ...
        try
        {
            if (!running)
            {
                NetworkManager.Instance.StartServer(int.Parse(((TextBox) GetControlFromType(typeof(TextBox), 0)).Text),
    ...
    public override void DrawScreen(SpriteBatch spriteBatch, GameTime gameTime)
    {
        ...
        spriteBatch.DrawCenteredString(Fonts.Hoog36, "Create Server", new Vector2(640, 50), Color.White);
        spriteBatch.DrawCenteredString(Fonts.Hoog12, Message, new Vector2(640, 135),
            running ? Color.Green : Color.Gray);
        spriteBatch.DrawStringWithShadow(Fonts.Hoog24, "Port:", new Vector2(400, 215), Color.White);
