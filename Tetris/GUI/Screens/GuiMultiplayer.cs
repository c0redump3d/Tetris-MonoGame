...
public class GuiMultiplayer : GuiScreen
{
    public override void SetUp()
    {
        ...
        Gui.Instance.MultiplayerMessage =
            NetworkManager.Instance.Connected
                ? $"Connected to {(NetworkManager.Instance.IsServer() ? "client" : "host")}"
                : "Not connected";
        AddControl(new Button(new Vector2(640, 420), "Connect", Fonts.Hoog28));
        AddControl(new Button(new Vector2(640, 510), "Create Server", Fonts.Hoog48));
        AddControl(new Button(new Vector2(640, 610), "Back", Fonts.Hoog48));
        ((Button) GetControlFromType(typeof(Button), 0)).OnClick += TryConnect;
        ((Button) GetControlFromType(typeof(Button), 1)).OnClick +=
            s => Gui.Instance.SetCurrentScreen(new GuiCreateServer());
    ...
    private void TryConnect(object sender)
    {
        ...
        catch (Exception)
        {
            ...
            Gui.Instance.AddDebugMessage("Invalid ip address.");
        }
    }
    ...
    {
        if (NetworkManager.Instance.Connected && ((Button) GetControlFromType(typeof(Button), 1)).Enabled &&
            !NetworkManager.Instance.IsServer())
        {
            ((Button) GetControlFromType(typeof(Button), 1)).Enabled = false;
            ((Button) GetControlFromType(typeof(Button), 0)).Text = "Disconnect";
        }
        else if (!NetworkManager.Instance.Connected && !((Button) GetControlFromType(typeof(Button), 1)).Enabled &&
                 !NetworkManager.Instance.IsServer())
        {
    ...
    public override void DrawScreen(SpriteBatch spriteBatch, GameTime gameTime)
    {
        ...
        spriteBatch.DrawCenteredString(Fonts.Hoog36, "Multiplayer", new Vector2(640, 50), Color.White);
        spriteBatch.DrawStringWithShadow(Fonts.Hoog24, "IP Address and Port:", new Vector2(400, 215), Color.White);
        spriteBatch.DrawStringWithShadow(Fonts.Hoog24, "Password:", new Vector2(400, 315), Color.White);
        spriteBatch.DrawCenteredString(Fonts.Hoog12, Gui.Instance.MultiplayerMessage, new Vector2(640, 135),
            NetworkManager.Instance.Connected ? Color.Green : Color.Gray);
